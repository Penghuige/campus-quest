...
Durable audit (PR #2 hardening step 7 + §30 completion pass, migrations 0014/0016): `audit_logs` is append-only (the sole insert path is `AuditLogWriter.append`, flush-only into the caller's transaction; no UPDATE/DELETE anywhere; actor has no FK — rows outlive user deletion) and carries the §30 field set: `before_snapshot`/`after_snapshot` (sanitized structured JSON — PII never enters snapshots, pinned by negative assertions), `ip_address`, `request_id` (both threaded from the transport via `AuditContext.from_request`: safe X-Request-ID + direct client IP, proxy trust boundary documented), alongside `details` as additional context. Audited actions: `COMMUNITY_IDENTITY_REVEAL`, `REDEMPTION_APPROVE`/`_REJECT`/`_FULFILL`, `SUBMISSION_APPROVED`, `SUBMISSION_REVISION_REQUIRED`, `REWARD_LOCK_INVALIDATED`, `COMMENT_MODERATE_DELETED`, `COMMENT_ADMIN_HARD_HIDDEN`, `REWARD_REVERSAL`, `REPORT_DISMISSED`/`_HANDLED`, `SYSTEM_SETTING_UPDATED` — actual state transitions only; idempotent replays write nothing; DomainEvent streams are unaffected. `reward_redemptions.rejection_reason` persists the reject reason (closure review pts-F1) and surfaces on the Admin review DTO only; the student catalogue DTO no longer carries the dormant `requires_manual_review` flag (V1 reviews every redemption manually — G13 option 1; auto-timeout remains the recorded open product decision).

Cleanup deletion claims (PR #2 hardening pass 4b, migration 0017): retention deletion is a declarative claim — one conditional UPDATE re-evaluates every retain guard against CURRENT PostgreSQL state (not the scan snapshot) and RETURNs the winner; `submissions.cleanup_claimed_at` / `upload_intents.cleanup_deleted_at` carry the claim. Protection wins: claim→VALIDATING and claim→UNDER_REVIEW write points refuse with a typed 409 while an un-deleted claim exists (under the assignment-claim row lock); a lost claim makes zero provider calls, so protection established between discovery and deletion leaves the object alive. Orphan UploadIntent objects (expired-unfinalized, burned-without-submission) join the same cleanup job, deleted only after `intent.expires_at` (URL TTL < intent TTL today); finalized rows are untouched and missing objects are idempotent success. Known edges: a worker crash between claim and mark strands the claim (manual column clear; timeout takeover not authorized), and the claim commit landing inside a protection transaction's check→commit gap is a sub-millisecond inherent boundary of the single-statement claim (the review-named race — protection commits first — is fully closed). The claim conflict reuses `VALIDATION_ERROR` (a dedicated CONFLICT code is a Plan 08 registry candidate).
