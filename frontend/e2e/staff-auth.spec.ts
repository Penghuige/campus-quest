 ...
 *
 * STATUS: SPEC ONLY — NOT WIRED TO A RUNNER YET (the T2-T7 guard pattern).
 *
 * Playwright itself is installed by Plan 10; until then this file stays
 * invisible to the gates (`tsconfig.json` includes only `src/**`, ESLint
 * globally ignores `e2e/**`, `next build` never touches it). Every test
 * below is skipped unless CQ_E2E=1, so importing the file can never
 ...
 * - CQ_E2E_STAFF_INVITE_URL  the fixture endpoint that mints a single-use
 *                            staff invitation and returns `{"token": "..."}`
 *                            (default $CQ_E2E_API_URL/admin/staff/invitations —
 *                            the admin surface lands with its own stream;
 *                            adjust there, the T2 SEED_URL stance).
 *
...
const INVITE_URL =
  process.env.CQ_E2E_STAFF_INVITE_URL ?? `${API_URL}/admin/staff/invitations`;
...
test.describe("staff invitation acceptance (brief step 1)", () => {
  ...
  /** Mint one single-use invitation via the Plan 10 fixture. */
  async function freshInvitationToken(email: string): Promise<string> {
    const response = await api.post(INVITE_URL, {
      data: { email, role: "TEACHER" },
    });
    expect(
      response.ok(),
      `invitation minting failed (${response.status()}); check CQ_E2E_STAFF_INVITE_URL and the Plan 10 fixture`,
    ).toBe(true);
    const body = (await response.json()) as { token: string };
    expect(typeof body.token, "the fixture must return {token}").toBe("string");
    return body.token;
  }
  ...
  test("invite -> password -> TOTP confirm -> recovery codes once -> done", async ({ page }) => {
    ...
    const token = await freshInvitationToken(
      `e2e-staff-${Date.now()}@school.edu`,
    );
  ...
  test("a dead invitation token gets the uniform rejection copy", async ({ page }) => {
    ...
    await expect(page.getByRole("alert")).toContainText("邀请链接无效或已被使用");
  });
});
